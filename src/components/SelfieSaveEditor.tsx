...
export default function SelfieSaveEditor({accountEmail,accountName,documentId,initialFilename,initialPdfBase64,autoOpenPdfPicker,initialDraft,signerHint,businessProfile,onClose,onSave}:SelfieSaveEditorProps){
  ...
  async function commitParty(party:number){const mine=fields.filter(f=>f.party===party);if(!mine.length||mine.some(f=>!f.signed))return;const warning="Save Signer "+party+"'s complete portion? This locks the agreement text and this signer's answers. The drafter cannot edit this signed copy afterward.";if(await requestConfirm(warning)){setFields(v=>v.map(f=>f.party===party?{...f,committed:true}:f));notify(`Signer ${party} committed — document text is now locked`)}}
