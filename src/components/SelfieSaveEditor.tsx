...
export interface SelfieSaveEditorProps {
  ...
  /** Focused Collect Signatures flow: show the PDF, require a typed legal
   * name plus a finger/stylus signature, capture location automatically,
   * and expose one Save Signature action. */
...
export default function SelfieSaveEditor({accountEmail,accountName,documentId,initialFilename,initialPdfBase64,autoOpenPdfPicker,initialDraft,signerHint,customerPhone,customerEmail,businessProfile,autoCaptureSignatures,autoOpenSignSetup,signatureOnlyMode=false,onClose,onSave}:SelfieSaveEditorProps){
  ...
  const [setup,setSetup]=useState(!signatureOnlyMode && !autoOpenPdfPicker && !initialPdfBase64 && !initialDraft);
  ...
  const pendingImagePos=useRef<{x:number;y:number;page:number}|null>(null);
  const triggerImageUpload=()=>{if(contentLocked)return;pendingImagePos.current={x:menu?.x||90,y:menu?.y||180,page:targetPage()};setMenu(null);imageInputRef.current?.click();};
  const handleImageFileSelected=async(file:File)=>{
    try{
      const{base64,mimeType}=await downscaleImageToBase64(file,1000,0.82);
      const pos=pendingImagePos.current||{x:90,y:180,page:targetPage()};
      setObjects(v=>[...v,{id:newId(),kind:"image",page:pos.page,x:pos.x,y:pos.y,w:220,h:160,value:`data:${mimeType};base64,${base64}`,scale:1}]);
    }catch{
      notify("Couldn't read that image. Try a different file.");
    }
    pendingImagePos.current=null;
  };
  const [isPriceBookOpen,setIsPriceBookOpen]=useState(false);
  const openPriceBook=()=>{if(contentLocked)return;pendingImagePos.current={x:menu?.x||90,y:menu?.y||180,page:targetPage()};setMenu(null);setIsPriceBookOpen(true);};
  const handlePricingModelPicked=(item:{description:string;quantity:number;unitPrice:number})=>{
    const pos=pendingImagePos.current||{x:90,y:180,page:targetPage()};
    setObjects(v=>[...v,{id:newId(),kind:"text",page:pos.page,x:pos.x,y:pos.y,w:220,h:40,value:`${item.description} — $${(item.quantity*item.unitPrice).toLocaleString()}`,scale:1,fontSize:12,fontFamily:"Arial, Helvetica, sans-serif"}]);
    pendingImagePos.current=null;
  ...
  function saveMark(){
    if(!active||!signerName.trim()||!consent||(signMethod==="drawn"&&!drawnSignature))return;
    const fieldId=active;
    const video=videoRef.current;
    let image="";
    if(features.selfies&&video){const c=document.createElement("canvas");c.width=320;c.height=240;c.getContext("2d")?.drawImage(video,0,0,320,240);image=c.toDataURL("image/jpeg",.78)}
    const signatureImage=signMethod==="drawn"?drawnSignature:undefined;
    const now=new Date();
    const stamp=new Intl.DateTimeFormat("en-US",{dateStyle:"medium",timeStyle:"short"}).format(now);
    const centralStamp=new Intl.DateTimeFormat("en-US",{dateStyle:"medium",timeStyle:"short",timeZone:"America/Chicago"}).format(now)+" Central Time";
    const save=(coords="")=>{
      makeRoomForSignedField(fieldId,!!image);
      const nextFields=fields.map(f=>f.id===fieldId?{...f,signed:true,committed:signatureOnlyMode,name:signerName.trim(),image,signatureImage,stamp,centralStamp,coords}:f);
      setFields(nextFields);
      streamRef.current?.getTracks().forEach(t=>t.stop());streamRef.current=null;
      setActive(null);setDrawnSignature("");
      if(signatureOnlyMode){setFinalLocked(true);void saveFocusedSignature(nextFields)}
      else notify("Signing field completed — review before committing");
    };
    if(features.location&&navigator.geolocation)navigator.geolocation.getCurrentPosition(p=>save(`${p.coords.latitude.toFixed(5)}, ${p.coords.longitude.toFixed(5)}`),()=>save("Location unavailable"));else save("Location unavailable");
  }
  ...
  async function currentPdfBytes():Promise<Uint8Array>{
    const hasFreeformContent=header.trim()||footer.trim()||clauses.length>0||objects.some(o=>o.value?.trim()&&o.value!=="Paste image URL"&&o.value!=="Paste link URL"&&o.value!=="Paste video URL");
    const freeformObjects=objects.map(o=>({kind:o.kind,page:o.page||1,x:o.x,y:o.y,w:o.w,h:o.h,value:o.value,fontSize:o.fontSize,fontFamily:o.fontFamily,color:o.color}));
    if(sourcePdfBytes){
      if(!hasFreeformContent)return sourcePdfBytes;
      const addendum=await buildFreeformDocumentPdf({filename,header,footer,clauses,objects:freeformObjects},business);
      return mergePdfs([sourcePdfBytes,addendum]);
    }
    return buildFreeformDocumentPdf({filename,header,footer,clauses,objects:freeformObjects},business);
  }
  async function saveFocusedSignature(nextFields:SignField[]){
    setExporting(true);
    try{
      ...
      onSave(docId,finalName,{
        ...
        signingOptions:{features,header,footer,clauses,fields:nextFields,placements,pageCount,hasImportedPdf:pdfPages.length>0}
      });
    }catch(error){console.error(error);setFinalLocked(false);notify("Could not save the signature. Try again.")}
  ...
  return createPortal(<div className="selfiesave-editor-root"><main className="app-shell">
    ...
    <section className={`workspace ${signatureOnlyMode?"signature-only-workspace":""}`}>{!signatureOnlyMode&&<aside className="sidebar"><div className="side-head"><h2>Document setup</h2></div><label>File name<input value={filename} disabled={contentLocked} onChange={e=>setFilename(e.target.value)}/></label><p className="fixed-name">Final file: <strong>{filename||"Untitled"}.pdf</strong></p><hr/><h3>Insert anywhere</h3><button className="insert" onClick={()=>addObject("text")} disabled={contentLocked}><Icon>T</Icon><span><strong>Free text box</strong><small>Type directly on page</small></span><b>＋</b></button><button className="insert" onClick={triggerImageUpload} disabled={contentLocked}><Icon>▧</Icon><span><strong>Upload image</strong><small>From your device</small></span><b>＋</b></button><button className="insert" onClick={openPriceBook} disabled={contentLocked}><Icon>💲</Icon><span><strong>Add Flat Rate Pricing Model</strong><small>Insert from the Price Book</small></span><b>＋</b></button><button className="insert" onClick={addClause} disabled={contentLocked}><Icon>§</Icon><span><strong>Contract clause</strong><small>Numbered text field</small></span><b>＋</b></button><button className="insert" onClick={()=>addField("signature")} disabled={contentLocked}><Icon>⌁</Icon><span><strong>Signature line</strong><small>Assign any signer</small></span><b>＋</b></button><button className="insert" onClick={()=>addField("initials")} disabled={contentLocked}><Icon>Ab</Icon><span><strong>Initials line</strong><small>Assign any signer</small></span><b>＋</b></button><button className="insert" onClick={()=>setSetup(true)} disabled={contentLocked}><Icon>⚙</Icon><span><strong>Evidence options</strong><small>Choose document requirements</small></span><b>›</b></button>{contentLocked&&<div className="security"><Icon>🔒</Icon><p><strong>Signed copy protected</strong><br/>Editing is disabled because a signer committed. Make a new unsigned version for any changes.</p></div>}</aside>}
      <section ref={editorRef} className="editor-wrap" onScroll={growPages}><div className="editor-tools"><span>{signatureOnlyMode?"Review the PDF and tap the signature box":contentLocked?"Signed document viewer":"Free-form document editor"}</span><div><button type="button" onClick={zoomOut} disabled={zoom<=0.4} aria-label="Zoom out">−</button><button type="button" onClick={zoomReset} aria-label="Reset zoom">{Math.round(zoom*100)}%</button><button type="button" onClick={zoomIn} disabled={zoom>=2.5} aria-label="Zoom in">＋</button><select disabled={contentLocked}><option>Georgia</option><option>Arial</option></select><select aria-label="Font size" disabled={contentLocked||!selectedTextObject} value={selectedFontSize} onChange={e=>setSelectedFontSize(Number(e.target.value))}>{Array.from({length:30},(_,index)=>index+1).map(size=><option key={size} value={size}>{size} pt</option>)}</select><input aria-label="Font color" type="color" disabled={contentLocked||!selectedTextObject} value={selectedTextColor} onChange={e=>setSelectedTextColor(e.target.value)} style={{width:28,height:28,padding:0,border:"1px solid #d7e3ee",borderRadius:6,cursor:selectedTextObject?"pointer":"not-allowed"}}/><button disabled={contentLocked}><b>B</b></button><button disabled={contentLocked}><i>I</i></button></div><span>{selected&&!contentLocked?"Use the blue Grab to move tab":""}</span></div>
        ...
        <div className="selfiesave-action-dock">
          {signatureOnlyMode
            ? <footer className="actionbar signature-only-actionbar"><strong>{exporting?"Saving signature…":"Tap the signature box on the PDF. Type your name, sign with your finger, then tap Save Signature."}</strong></footer>
            : <footer className="actionbar" role="toolbar" aria-label="Document actions"><div><strong>{fields.length?`${fields.filter(f=>f.committed).length} of ${fields.length} fields committed`:"No signatures requested"}</strong><span><i style={{width:`${fields.length?fields.filter(f=>f.committed).length/fields.length*100:0}%`}}/></span><small>{fields.length===0?"Save a draft or a completed copy any time -- signatures are optional.":contentLocked&&!complete?"Agreement text locked; remaining signers may complete only their assigned fields.":complete?"All parties committed. Ready for final lock.":"Each signer must complete and commit every assigned field."}</small></div><button className="save-draft" disabled={exporting} onClick={()=>exportPdf(false)}>{exporting?"Building PDF…":"💾 Save Draft"}</button><button className="save-draft" disabled={savingCompleted} onClick={saveCompleted}>{savingCompleted?"Building PDF…":"✅ Save Completed Document"}</button><button className="save-draft" disabled={contentLocked} onClick={openCollectSignatures}>✍️ Collect Signatures</button><button className="save-draft" onClick={()=>{setSendBody("");setSendOpen(true)}}>📤 Send</button><button className="finalize" disabled={!complete||finalLocked||exporting} onClick={finalize}>🔒 Save final signed copy</button></footer>}
        </div>
        {/* transform-origin is "top left", not "top center" -- centering the
            ...
            offset, so roughly half of that growth (whatever extends left of
            center) became permanently unreachable and the zoomed page looked
            clipped/broken. Anchoring at top-left keeps all growth in the
            positive-scroll direction (right/down), fully reachable by
            scrolling normally. */}
        <div className="document-pages" style={{transform:`scale(${zoom})`,transformOrigin:"top left"}}>{Array.from({length:pageCount},(_,pageIndex)=>{const page=pageIndex+1,pdfPage=pdfPages[pageIndex];return <article ref={page===1?paperRef:undefined} key={page} className={`paper ${pdfPage?"imported-pdf-page":""} ${finalLocked?"paper-locked":""}`} style={pdfPage?{aspectRatio:`${pdfPage.width} / ${pdfPage.height}`}:{}} onPointerDown={e=>{if(!(e.target as HTMLElement).closest(".canvas-object"))setSelected(null)}} onDoubleClick={e=>openObjectMenu(e,page)}>
