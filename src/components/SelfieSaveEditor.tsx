...
export default function SelfieSaveEditor({accountEmail,accountName,documentId,initialFilename,initialPdfBase64,autoOpenPdfPicker,initialDraft,signerHint,businessProfile,onClose,onSave}:SelfieSaveEditorProps){
  ...
  async function loadPdf(file:File,fromInitial=false){
    ...
    if(!fromInitial&&(clauses.length||fields.length||objects.length||pdfPages.length)&&!confirm("Open this PDF as a new document? Your current unsigned work will be replaced."))return;
    setLoadingPdf(true);
    try{
  ...
  async function exportPdf(finalCertificate:boolean){
    ...
    try{
      ...
      notify(finalCertificate?"Final signed PDF saved to Documents and downloaded":"PDF saved to Documents and downloaded");
    }catch(error){
      console.error(error);
